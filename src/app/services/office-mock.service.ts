...
export class OfficeMockService implements IOfficeService {
    ...
    getType(control: Word.ContentControl): ControlType {
        const n = Math.trunc((Math.random() * 3) + 1);
    ...
    getAllContentControls(): Promise<Word.ContentControlCollection> {
        return new Promise((resolve, reject) => {
            resolve({items: [{ title: 'Feld1', text: 'Hello World!' }]} as Word.ContentControlCollection);
        });
    }
    createContentControl(range: Word.Range, title?: string, tags?: string[], editable?: boolean): Promise<void> {
        throw new Error('Method not implemented.');
    }
